...
This document specifies the tests for **PTR-28 — See when a venue is free**. It records the planned behaviour to verify, the acceptance criteria each case addresses, and a separate execution record to complete after each run.
...
| [PTR-28-TC14](#ptr-28-tc14) | External roles cannot retrieve availability through the server function | AC5                 | Direct     |
...
The repository has authentication, venue and blocked-period tables, but no booking table or booking persistence. The cases can be reviewed now; booking-dependent execution remains deferred until PTR-31/PTR-33 supply the shared schema.

- **Venue records:** coordinate with [PTR-26](https://linear.app/is212-petra/issue/PTR-26/maintain-the-record-for-a-venue).
- **Demo and blocked-period data:** [PTR-59](https://linear.app/is212-petra/issue/PTR-59/seed-the-staff-accounts-and-demo-data-the-team-builds-against) supplies the local staff accounts, venues and blocked periods used by live checks.
...
- Product decision recorded: Technical Support Staff has `venueAvailability:read` only (view/read-only calendar access), with no calendar mutation capability. Confirm the product timezone, operating-hours treatment and adjacent-period boundary rule before adding assertions for those decisions.
...
The page is `/venues/availability`; its calendar reads use the venue server functions (`listVenues` and the availability schedule function). Keep pure input validation and schedule projection in unit tests. Exercise authenticated server-function and database behaviour through the live Playwright application because TanStack server functions require their runtime request context; do not substitute direct route-handler invocation as feature evidence. Do not invent a passing result when a required booking persistence prerequisite is unavailable.
...
**External roles cannot retrieve availability through the server function**
...
| Acceptance Criteria | Criterion addressed and type of coverage       | AC5 — direct acceptance coverage                                                                                                                                                                                                                                                                                |
| Test Scenario       | Succinct objective of the test case            | External roles cannot retrieve availability through the server function.                                                                                                                                                                                                                                        |
| Pre-conditions      | Conditions to fulfil before executing the test | P0 applies. The calendar server-function contract is agreed. Valid sessions exist for ATTENDEE-A and ORGANISER-A.                                                                                                                                                                                               |
| Test Steps          | Step-by-step procedure to execute the test     | 1. Authenticate as variant A and call the availability server function without loading the page.<br>2. Inspect the refusal and error status at the function boundary.<br>3. Repeat with variant B in a separate authenticated session.<br>4. Record sanitized input and refusal status; exclude session tokens. |
| Test Data           | Specific inputs used to conduct the test       | Variants A=ATTENDEE-A and B=ORGANISER-A. Valid calendar request: Venue A and R1.                                                                                                                                                                                                                                |
| Expected Result     | Expected result from the test                  | Both calls are refused with 403 Forbidden under the repository’s authorisation convention. No availability, booking or blocked-period data is returned. AC5 requires refusal; transport details are implementation-specific.                                                                                    |
| Planned Test Level  | Where the behaviour will be verified           | Browser against the live application                                                                                                                                                                                                                                                                            |
| Created By*         | Author of the test case                        | Codex, prepared for Yuhan Huang                                                                                                                                                                                                                                                                                 |
| Date of Creation*   | Date the test case was created                 | 12 September 2026                                                                                                                                                                                                                                                                                               |
...
| Test Data           | Specific inputs used to conduct the test       | Variants: A=missing venue; B=nonexistent venue ID; C=start date 2026-99-99; D=end date earlier than start (07 October to 05 October 2026); E=missing one date field.                                                                                                                                                    |
...
| Test Steps          | Step-by-step procedure to execute the test     | 1. Variant A: navigate directly to the calendar page while signed out.<br>2. Inspect the final URL and ensure no calendar data is displayed.<br>3. Variant B: call the availability server function for Venue A/R1 without a session.<br>4. Record the page result and direct function refusal separately. |
| Test Data           | Specific inputs used to conduct the test       | Anonymous user; Venue A; R1; agreed calendar page and availability server function.                                                                                                                                                                                                                        |
...
For future booking work, review the case specifications, confirm the open decisions, and agree the booking schema. Until PTR-31/PTR-33 add persistence, retain only live venue/blocked-period checks and authorization checks; do not claim AC3 or booking portions of AC2/AC4 from fixtures.

...
│   │   │   ├── records.server.ts # Server-only relationship-scoped read; never client-reachable
│   │   │   └── server-fns.ts # `listEvents` — session guard, then the read scopes per caller
...
│   │   │   │   └── reopenDraft.$id.tsx # Reopen a saved draft in the editor (PTR-12)
│   │   │   ├── settings.tsx  # Profile, linked providers, delete account
│   │   │   └── venues/
│   │   │       ├── index.tsx     # Role-gated (venue:read) — the catalogue
│   │   │       ├── availability.tsx # Role-gated (venue:read) — the venue calendar (PTR-28)
...
- **API route handlers** — `src/routes/api/upload-url.ts`, which calls `can()` next to its session check. Event access is a server function, so it runs in the pipeline above rather than reading the session itself.

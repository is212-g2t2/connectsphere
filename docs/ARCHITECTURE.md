...
`permissions.ts` stays pure data because the browser imports it too — a server import there fails `bun run build` alone. The session-aware half is `requirePermission(user, request)` in `src/features/auth/session.ts`: `Unauthorized` without a session, `Forbidden` without the permission. It throws an `AuthorizationError` carrying the status the refusal deserves (401 or 403) rather than a bare `Error`, because criterion 2 asks for an authorisation error and not a generic failure. It stays an `Error` rather than a `Response` so the pure `handle*` functions can still be called directly from tests; `refuseWithStatus` in `server-fns.ts` converts it at the server-function boundary, where a thrown `Response` is returned verbatim. Hiding a control is presentation, not enforcement, so each entry point checks for itself:

- **Server functions** — each `handle*` in `src/features/notes/server-fns.ts`, which are POST endpoints reachable without ever loading the route.
- **API route handlers** — `src/routes/api/upload-url.ts`, next to its session check.
- **The interface** — `src/routes/dashboard.tsx` asks the same `can()` before rendering a control.

...
- `src/db/schema.ts` — Application domain schemas. Re-exports the auth tables; holds `eventRequests` (PTR-9, PTR-10, PTR-11, PTR-13), the venue catalogue (`venues`, `venue_unavailability`, PTR-26), and the event access tables (`events`, `eventCoordinators`, `venueRequests`, `equipmentRequests`, `eventRegistrations`, PTR-8).

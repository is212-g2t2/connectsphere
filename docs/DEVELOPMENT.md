...
- `src/db/schema.ts` — Application domain schemas. Re-exports the auth tables; holds `eventRequests` (PTR-9, PTR-10, PTR-11, PTR-13), Coordinator handover history and notifications (`eventAssignments`, `eventAssignmentNotifications`, PTR-16), and the venue catalogue (`venues`, `venue_unavailability`, PTR-26). The equipment tables arrive with the stories that build them.

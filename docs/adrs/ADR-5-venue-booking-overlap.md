...
- Drizzle's migrator stores each migration's hash but decides what to run from the timestamp alone, so editing an already-applied migration never re-runs it: a database that applied 0018 without 0019 still receives 0019 normally, but a change made to 0019 after it was applied needs a manual constraint update.

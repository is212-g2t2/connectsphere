...
- **One image serves both environments.** All baked `VITE_*` values are identical; the browser's Sentry environment is derived from the hostname at runtime. That is what allows a fast-forwarded `staging` → `main` merge to promote the exact digest staging tested rather than a rebuild. (The fast-forward rule is superseded by [ADR-4](./ADR-4-trunk-based-main.md); the one-image property stands.)

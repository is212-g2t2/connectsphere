...
- **Venue availability** (PTR-28): Event Coordinators, Venue Staff and Technical Support Staff can read the calendar through protected venue server functions; Technical Support Staff is view-only. The live implementation reads venues and recorded unavailability. Booking persistence and AC3 remain deferred to PTR-31/PTR-33, so fixture-only booking assertions are not live evidence.

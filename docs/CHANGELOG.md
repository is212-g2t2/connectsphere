...
- **Automatic Coordinator assignment** (PTR-15): submitting a request assigns it, in the same transaction, to the Event Coordinator currently handling the fewest submitted requests (earliest account on a tie), recording who and when; a single column and two database CHECKs keep it to at most one Coordinator and to submitted requests only. The Coordinator's name and email appear on the organiser's list and detail. When no Coordinator exists the submission is still recorded and waits at `/coordination`, the Coordinators' workspace behind the new `event_request:coordinate` permission, until PTR-16 lets one pick it up. Notifying both parties (AC4) waits for PTR-55's notification mechanism.

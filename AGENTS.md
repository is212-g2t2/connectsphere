...
- `#/db/schema` is the trap, because unlike `#/db` it does **not** fail `bun run build`: it just silently serves the entire database schema, Better Auth tables included, to the browser. `tests/unit/client-bundle-safety.test.ts` is the only thing that catches it — keep its `SERVER_ONLY_IMPORT` pattern current when adding a server-only module.
- Instead, reach server dependencies dynamically inside `.handler()` — or a middleware's `.server()` callback, as `src/features/auth/session.ts` does — via `await import("#/db")`, import server types with `import type`, and require injected dependencies in exported helpers (e.g. `database: Database`).
- Do **not** give a client-reachable module the `<feature>.server.ts` suffix. `@tanstack/start-plugin-core`'s import-protection plugin denies `**/*.server.*` in the client environment, so the first route that imports it fails `bun run build` — and only `build`, not `type:check` and not the test suites. That suffix is only for modules nothing client-reachable imports, like `event-requests/drafts.server.ts`.
